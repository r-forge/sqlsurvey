...
setClass("MonetDriver", representation("DBIDriver", identifier.quote="character", jdrv="jobjRef"))

MonetDB <- function(driverClass="nl.cwi.monetdb.jdbc.MonetDriver", classPath='', identifier.quote='"') {
  .jinit(classPath)
  .jaddClassPath(system.file("java", "RJDBC.jar", package="RMonetDB"))
  ...
  new("MonetDriver", identifier.quote=as.character(identifier.quote), jdrv=jdrv)
}
...
setMethod("dbListConnections", "MonetDriver", def=function(drv, ...) { warning("JDBC driver maintains no list of acitve connections."); list() })

setMethod("dbGetInfo", "MonetDriver", def=function(dbObj, ...)
...
setMethod("dbUnloadDriver", "MonetDriver", def=function(drv, ...) NULL)

setMethod("dbConnect", "MonetDriver", def=function(drv, url, user='', password='', ...) {
  ...
  new("MonetConnection", jc=jc, identifier.quote=drv@identifier.quote)},
          valueClass="MonetConnection")

### JDBCConnection

setClass("MonetConnection", representation("DBIConnection", jc="jobjRef", identifier.quote="character"))

setMethod("dbDisconnect", "MonetConnection", def=function(conn, ...)
...
setMethod("dbSendQuery", signature(conn="MonetConnection", statement="character"),  def=function(conn, statement, ..., list=NULL) {
...
setMethod("dbSendUpdate",  signature(conn="MonetConnection", statement="character"),  def=function(conn, statement, ..., list=NULL) {
  if(!is.null(list) || length(list(...))){
    ...
    s <- .jcall(conn@jc, "Ljava/sql/Statement;", "createStatement", check=FALSE)
    .verify.JDBC.result(s, "Unable to create JDBC statement "," ")
  }
  if(!is.null(list) || length(list(...)))
...
setMethod("dbGetQuery", signature(conn="MonetConnection", statement="character"),  def=function(conn, statement, ...) {
  r <- dbSendQuery(conn, statement, ...)
  fetch(r, -1)
})

setMethod("dbGetException", "MonetConnection",
          def = function(conn, ...) list()
          , valueClass = "list")

setMethod("dbGetInfo", "MonetConnection",
          def = function(dbObj, ...) list() )

setMethod("dbListResults", "MonetConnection",
          def = function(conn, ...) { warning("JDBC maintains no list of active results"); NULL }
          )

setMethod("dbListTables", "MonetConnection", def=function(conn, pattern="%", ...) {
...
setMethod("dbExistsTable", "MonetConnection", def=function(conn, name, ...) (length(dbListTables(conn, name))>0))

setMethod("dbRemoveTable", "MonetConnection", def=function(conn, name, ...) dbSendUpdate(conn, paste("DROP TABLE", name))==0)

setMethod("dbListFields", "MonetConnection", def=function(conn, name, pattern="%", ...) {
  md <- .jcall(conn@jc, "Ljava/sql/DatabaseMetaData;", "getMetaData", check=FALSE)
  .verify.JDBC.result(md, "Unable to retrieve JDBC database metadata")
...
setMethod("dbReadTable", "MonetConnection", def=function(conn, name, ...)
          dbGetQuery(conn, paste("SELECT * FROM",.sql.qescape(name,TRUE,conn@identifier.quote))))

setMethod("dbDataType", signature(dbObj="MonetConnection", obj = "ANY"),
...
setMethod("dbWriteTable", "MonetConnection", def=function(conn, name, value, overwrite=TRUE, ...) {
...
setMethod("dbCommit", "MonetConnection", def=function(conn, ...) {.jcall(conn@jc, "V", "commit"); TRUE})
setMethod("dbRollback", "MonetConnection", def=function(conn, ...) {.jcall(conn@jc, "V", "rollback"); TRUE})
